...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Benchmark reasoning SFT checkpoints on HotpotQA (EM/F1 on extracted final answers)."
    )
    ap.add_argument("--model_path", type=str, required=True, help="Finetuned model/checkpoint path")
    ap.add_argument("--base_model", type=str, default=None, help="Base model path when model_path is a LoRA adapter")
    ...
    ap.add_argument("--max_samples", type=int, default=1000, help="0 means all")
...
def generate_batch(
    ...
    eos_ids: List[int] = [tokenizer.eos_token_id] if tokenizer.eos_token_id is not None else []
    ...
    eos_arg: Any = eos_ids[0] if len(eos_ids) == 1 else eos_ids if eos_ids else None
    ...
    for i in range(out.size(0)):
        ...
        raws.append(tokenizer.decode(gen_ids, skip_special_tokens=False).strip())
    return raws
...
def main() -> None:
    ...
    total = len(ds)
    for start in tqdm(range(0, total, args.batch_size), desc="HotpotQA reasoning eval", unit="batch"):
        ...
        for i, question in enumerate(questions):
            ...
            context = build_hotpot_context(ex, mode=args.context_mode, max_context_chars=args.max_context_chars)
            contexts.append(context)
            prompts.append(build_prompt(query=str(question), context=context, args=args))
        ...
        for i, raw in enumerate(raws):
            ...
            em = answer_em(prediction=answer, gold=gold)
            f1 = answer_f1(prediction=answer, gold=gold)
            sum_em += em
            sum_f1 += f1
            n += 1
